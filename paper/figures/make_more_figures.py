...
def fig_pipeline():
    ...
    box(9, 4, 82, 16,
        ...
        T_ORACLE, fontsize=7.0)
    # drop from below box (e) straight onto the right end of the (f) strip,
    # staying right of box (d) so no box overdraws the arrow
    arrow(90, y0 - 2, 85, 21, color="#888888", lw=0.7, rad=0.18)
...
def fig_roadmap():
    ...
    box(2, 6, 32, 40,
        ...
        T_DEGR, ec=C_DEGR, fs=7.2, fw="bold")
    # arrow from closed --> open
    arrow(35, 26, 38, 26, color="#888888", lw=1.0)

    # ---- right top: Road 1 ----
    box(39, 26, 59, 20,
        "Road 1  \u2014  degradation-aware training / adaptation\n\n"
        "keep the agent discriminative under corruption\n"
        "(robust pre-training, adapter, contrastive heads)",
        T_BASE, ec=C_BASE, fs=7.0, fw="normal")

    # ---- right bottom: Road 2 ----
    box(39, 4, 59, 20,
        "Road 2  \u2014  a degradation-invariant sensing channel\n\n"
        "fuse RGB with depth / LiDAR under degraded RGB\n"
        "(DepthVeto probe here; remedies need learned fusion)",
        T_POS, ec=C_POS, fs=7.0, fw="normal")
