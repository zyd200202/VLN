...
Layout (double-column banner, two rows):
  ...
  (c) real Habitat top-down episode + the aggregate 24-pt reach-to-success gap
...
def panel_forest(fig, slot):
    ...
    deployable = [
        ...
        ("R-Weight",   "300", n300_b, RUNS / "redesign_n300/n300_M1g_low_light_s4_seed0.csv"),
        ("NearVerify", "150", n150_b, uni / "u_CRV_low_light_s4_seed0_N150.csv"),
        ...
        ("ReObserve",  "150", n150_b, uni / "u_MUAP_low_light_s4_seed0_N150.csv"),
        ("DepthVeto",  "150", n150_b, uni / "u_DXCV_low_light_s4_seed0_N150.csv"),
    ]
    ...
    ax.set_xticks(xpos)
    ax.set_xticklabels(
        [f"{name} ($N{{=}}{n}$)" for name, n, _ in rows],
        fontsize=6.2, rotation=24, ha="right", rotation_mode="anchor")
...
def main():
    ...
    bb = dict(boxstyle="round,pad=0.22", fc=(1, 1, 1, 0.94),
              ec="#AAAAAA", lw=0.4)
    ax.text(0.03, 0.03,
            "$N{=}300$:  reach 61%  vs.  succeed 37%\n"
            "24-pt gap no test-time fix closes",
