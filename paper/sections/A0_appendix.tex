\section{The Instrument, Stimuli, and Episode-Level Traces}
\label{app:instrument}

This appendix collects figures referenced from the main text: the
diagnosis instrument (Figure~\ref{fig:pipeline}) and the per-episode
mechanism decomposition (Figure~\ref{fig:decomp}).

\begin{figure*}[!t]
\centering
\includegraphics[width=0.86\textwidth]{figures/fig_decomp.pdf}
\caption{\textbf{Inside the commitment bottleneck (per-episode logs, paired
$N{=}300$ low-light).}
\textbf{(a)}~ECDF of distance at the \emph{first commit}: B0, RES, and
\textsc{OracleGate} coincide; ${\sim}70\%$ of first commits fire ${>}3$\,m
from the goal. \textbf{(b)}~Reliability $r$ at the commit frame: RES
shifts the median $0.40\to1.00$, yet commit-to-success conversion moves
only $0.42\to0.43$. \textbf{(c)}~Paired min-distance (B0 vs.\ RES) with
flips coloured: rescued episodes (teal) were already at the $1$\,m margin
under B0 (median $1.1$\,m); the intervention shuffles the margin rather
than repairing far failures.}
\label{fig:decomp}
\end{figure*}
...
\caption{\textbf{The diagnosis instrument.} A
...
while the matched $N{=}150$ comparison of Table~\ref{tab:cross-corruption}
...
The diagnosis instrument is illustrated in Figure~\ref{fig:pipeline}
(App.~\ref{app:instrument}). For each (severity, seed) pair, every arm is evaluated on the same fixed
...
\item \textbf{R-Weight (reliability-gated reweight; code name
\texttt{M1g})}: $\alpha{=}1.0$, $\beta_{\max}{=}0.6$,
$\tau_{\text{gate}}{=}0.9$.
...
\item \textbf{NearVerify (close-range recall verifier; code name
\texttt{CRV})}: rel-gate $0.90$, GLEE
conf $0.10$, floor $0.12$, mask area $\geq 5000$\,px, frames $1$, max $3$
verifications, centre tolerance $0.18$.
\item \textbf{ReObserve (code name \texttt{MUAP})}: target hold-frames $8$,
min frames $3$, match distance $0.6$\,m; re-observe mode,
$\tau_{\text{commit}}{=}0.9$, max re-observations $3$, confidence margin
$0.35$.
\item \textbf{FUSE (multi-view)}: $\beta{=}0.10$, IoU $0.5$, reliability
floor $0.2$, identity-mask MAD $1.0$.
\item \textbf{DepthVeto (code name \texttt{DXCV})}: rel-gate $0.85$, masked depth median in
...
are RES ($b_{01}{=}40$, $b_{10}{=}28$), OracleGate ($40$, $23$), and R-Weight
($43$, $31$). At the $N{=}150$ unified queue on low-light severity~$4$, ROUTER
reports $b_{01}{=}21$, $b_{10}{=}19$;
REVOKE $b_{01}{=}11$, $b_{10}{=}29$;
NearVerify $b_{01}{=}18$, $b_{10}{=}15$;
ReObserve  $b_{01}{=}19$, $b_{10}{=}37$;
FUSE   $b_{01}{=}20$, $b_{10}{=}22$;
DepthVeto  $b_{01}{=}10$, $b_{10}{=}36$.
Three arms (REVOKE, ReObserve, DepthVeto) thus break roughly twice as many
episodes as they rescue or more, matching the significant negative
$\Delta$SR reported in the sweep panel of Figure~\ref{fig:teaser} (right).
...
than consulting it at inference, as our DepthVeto probe does).}
\label{fig:roadmap}
\end{figure*}

\input{sections/_qwen_sweep_block}

\input{sections/_og_vignettes_block}
...
directly paired against the same B0 used in Table~\ref{tab:sweep}.}
\label{fig:road1}
\end{figure*}
...
degradation is low-light severity~$4$ applied through the same
...
the paired $N{=}150$ low-light severity~$4$ evaluation against the same B0
seed used in Table~\ref{tab:sweep}.
...
deployable test-time arm of Table~\ref{tab:sweep} while leaving the entire training
...
deterministic ordering) for $N{=}300$ low-light severity~$4$~seed~$0$; (iii) all
per-episode CSVs used in this paper (success, SPL, distance-to-goal, goal
category); (iv) the per-arm monkey-patch sources against InstructNav
(frozen backbone) for RES, R-Weight, OracleGate, REVOKE, NearVerify,
ReObserve, FUSE, and DepthVeto (code names in the release:
\texttt{M1g}, \texttt{CRV}, \texttt{MUAP}, \texttt{DXCV});
