...
the perceptual part of this loop to large vision--language models (VLMs): a
...
however, the camera rarely delivers clean frames: corridors are dimly lit,
lenses fog up, and motion blurs the image. Under these conditions, the same
agents lose much of their success rate: in our measurements, low light alone
halves the success rate of a representative zero-shot navigator.

A natural way to address this problem is to treat it as an image-quality
issue and insert a restoration front-end, such as low-light enhancement or
denoising, between the camera and the policy \citep{guo2020zerodce,
wu2023uretinex}. The underlying assumption is that once the corrupted frame is
made to look clean again, the downstream navigator will behave as if the
degradation were absent. Whether this assumption holds, however, has not been
examined directly for VLM-driven navigators. We find that it does not, and we
use a controlled diagnosis to show \emph{why}. This is a
diagnosis-and-benchmark paper: our contribution is the measurement
instrument and the resulting map of what does not work, not a new agent.
...
does reduce success: under low light the success rate falls by roughly half.
...
per-frame oracle that picks the best of the degraded and restored views does no
...
single frame---a \emph{commitment}; the commit rate (${\sim}93\%$) and the
pure-exploration-failure rate (${\sim}6\%$) are essentially unchanged from
clean, but degradation corrupts the \emph{quality} of these irreversible
commitments. Decomposing the outcome as
$\text{SR}=\text{reachability}-\text{commit/stop loss}$, the agent walks within
one metre of the goal in $62\%$ of episodes yet succeeds in only $36\%$; it
commits at a frame reliability of $0.39$ versus $0.95$ on clean input, only
$40\%$ of its commitments lead to success, and once committed it stays locked
for ${\sim}12$ steps. The behaviour resembles a person who, catching a brief glimpse of a
doorway, strides toward it and keeps going long after it proves wrong: the agent
does not fail because it cannot see; it fails because it makes an irreversible
decision on a single unreliable frame and cannot revoke it.
...
oracle---an upper bound on any test-time arbitration over appearance---caps
at $+0.057$. A broader sweep across six causal families (appearance,
...
\emph{deployable} family---one that consults no privileged information at
test time---either lands at a non-significant ${\sim}0$ or
\emph{significantly hurts} success (Tab.~\ref{tab:sweep}), indicating an
irreducible commitment error that no single test-time channel can recover.
...
\item \textbf{Negative results.} A sweep of six causal intervention
  ...
  at $+0.057$; we further show that per-condition restoration effects can
  sign-flip between $N{=}40$ and $N{=}150$, so under-powered evaluations
  can conclude the wrong direction.
\item \textbf{Replication.} A cross-corruption replication on the same
  InstructNav+Qwen2-VL-7B pipeline (low-light, motion-blur, fog,
  Gaussian noise) that reproduces the same non-significant fingerprint,
  with $|\Delta\text{SR}|{\leq}0.05$ and sign-flipping across cells,
  establishing that the effect is not tied to any specific corruption
  channel. We release all settings, seeds, and negative results.
\end{itemize}
