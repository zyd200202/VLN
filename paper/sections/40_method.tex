...
any test-time arbitration over appearance. The instrument behind the test
(Figure~\ref{fig:pipeline}) is a paired protocol that exposes the same
frozen backbone to clean and degraded versions of identical episodes and
measures the per-episode difference.

\subsection{Per-Step Reliability Estimation}

\begin{figure}[!t]
\centering
\includegraphics[width=0.78\columnwidth]{figures/fig_calibration.pdf}
\caption{\textbf{Blind reliability estimator $r$} on the calibration probe
set. Clean frames score $r{\approx}1$; each structure-preserving corruption
lowers $r$ monotonically. Dashed line is the $r{=}0.9$ no-op gate above
which all reliability-gated candidates reduce to the baseline.}
...
(Fig.~\ref{fig:calibration}): clean $1.00$, low-light $0.65$, motion-blur
...
information---but it bounds the best a perfect router over restored versus
degraded appearance could ever achieve. If even \textsc{OracleGate} does not beat
blanket restoration, no appearance-level decision can.
...
The reliability-gated reweighting candidate (M1g) takes a convex mixture
...
below it ($\alpha{=}1$, $\beta_{\max}{=}0.6$), so on clean input M1g
reduces exactly to the backbone.

Together these candidates probe pixels (RES, OracleGate) and where-to-go
(M1g); \textsc{OracleGate} is an upper bound rather
than a method, so the collective failure to move success would be a
statement about \emph{where the headroom is}, not about weak baselines. The
Experiments section confirms this and sweeps four further families
(commitment timing/revocation, forced commitment, temporal/multi-view
aggregation, and a depth-based multimodal probe), so the headroom claim is
bounded across the full intervention space.
