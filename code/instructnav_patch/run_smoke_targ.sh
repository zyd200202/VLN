#!/bin/bash
# SMOKE: Task-Aware Restoration Gating (TARG) vs degraded baseline (B0sm), paired same-seed.
# TARG runs dual-path GLEE (degraded vs restored) and adopts the restored frame ONLY when it
# improves a downstream task utility (target conf + detection mass + memory consistency) by > margin.
# Goal: show decision-driven restoration beats both the degraded baseline AND blanket RES.
...
OUTDIR=/root/autodl-tmp/DRPhysNav/runs/smoke_targ
LOG="$OUTDIR/smoke.log"; mkdir -p "$OUTDIR"
EPISODES="${SMOKE_EPISODES:-40}"; SEV=4; SPLIT=val
export DRPN_WANDB=1 DRPN_WANDB_PROJECT=DRPhysNav DRPN_WANDB_GROUP=smoke_targ
...
run_one() {
  ...
  case "$ARM" in
    B0sm) : ;;                                  # degraded baseline (no intervention)
    TARG) export DRPN_USE_TARG=1 DRPN_TARG_LOG=1 ;;  # decision-driven restoration gating
...
# Paired vs the EXISTING B0sm baseline (smoke_thf_uap/smoke_B0sm_low_light_s4_seed0.csv, N=40, SR=0.350);
# copied into this dir for paired McNemar. Only the TARG arm needs running.
...
QUEUE=("TARG low_light 0")
echo "########## SMOKE TARG START $(date) N=$EPISODES ##########" | tee -a "$LOG"
for item in "${QUEUE[@]}"; do run_one $item; done
echo "ALL_DONE smoke_targ $(date)" | tee -a "$LOG"
