#!/bin/bash
# SMOKE: blanket restoration (RES) vs baseline, with M1_LOG to diagnose per-frame restoration benefit.
# Establishes whether restoration has CONDITIONAL signal (-> basis for decision-driven restoration).
...
OUTDIR=/root/autodl-tmp/DRPhysNav/runs/smoke_res
LOG="$OUTDIR/smoke.log"; mkdir -p "$OUTDIR"
EPISODES="${SMOKE_EPISODES:-40}"; SEV=4; SPLIT=val
export DRPN_WANDB=1 DRPN_WANDB_PROJECT=DRPhysNav DRPN_WANDB_GROUP=smoke_res
...
base_env() {
  ...
  export DRPN_USE_DDA=0 DRPN_DEGRADE_SEVERITY=$SEV DRPN_MOTIV_LOG=1
}
run_one() {
  ...
  case "$ARM" in
    RES) export DRPN_USE_RESTORE=1 DRPN_M1_LOG=1 ;;   # blanket restoration + PSNR diagnostic
...
QUEUE=("RES low_light 0")
echo "########## SMOKE RES START $(date) N=$EPISODES ##########" | tee -a "$LOG"
for item in "${QUEUE[@]}"; do run_one $item; done
echo "ALL_DONE smoke_res $(date)" | tee -a "$LOG"
