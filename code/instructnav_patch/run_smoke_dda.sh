...
OUTDIR=/root/autodl-tmp/DRPhysNav/runs/smoke_dda
...
export DRPN_WANDB_GROUP=smoke_dda
...
en_dda() {
  ...
  export DRPN_USE_DDA=1 DRPN_DDA_MIN_RGATE=1.0
...
echo "############ SMOKE DDA START $(date) split=$SPLIT N=$EPISODES sev=$SEV ############" | tee -a "$LOG"
for item in "${QUEUE[@]}"; do
  run_one $item
done
echo "ALL_DONE smoke_dda $(date)" | tee -a "$LOG"
